    ...
    <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', background: 'var(--cth-paper-200)', padding: 10, display: 'flex', flexDirection: 'column', gap: 10, fontFamily: 'var(--cth-font-mono)' }}>
      {waiting.length === 0 && (
        <div style={{ textAlign: 'center', padding: '24px 12px', color: 'var(--cth-ink-500)', fontSize: 12 }}>
          今、あなたの確認が必要なことはありません。🌿<br />
          <span style={{ fontSize: 11, color: 'var(--cth-ink-300)' }}>
            回答が必要な質問や、あなただけが行える作業でタスクが止まると、
            ここ（およびチーム画面の「確認事項」ボード）に表示されます。
          </span>
        </div>
      )}
      {waiting.map((t) => {
          ...
          <div key={t.id} style={{
            ...
            <div style={{
              ...
              <button
                ...
                title="回答せずに確認事項ボードから外す（履歴は保持されます）"
                aria-label="この確認事項を閉じる"
                style={{
            ...
            <div style={{ padding: 9, display: 'flex', flexDirection: 'column', gap: 8 }}>
              ...
              <textarea
                ...
                rows={3}
                placeholder="回答、または完了した結果を入力…（Ctrl+Enterで送信）"
                style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                >
                  {sending === t.id ? '送信中…' : '回答して再開'}
                </PixelButton>
                {(t.humanQA?.filter((e) => e.a).length ?? 0) > 0 && (
                  <button
                    onClick={() => openTaskDetail(t.id)}
                    title="Q&A履歴を含むタスク詳細を開く"
                    style={{
                  ...
                  >
                    過去の回答 {t.humanQA!.filter((e) => e.a).length}件を表示
                  </button>
                )}
              </div>
              ...
              {stuck.length > 0 && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                  ...
                  {stuck.length > 6 && (
                    <div style={{ paddingLeft: 14, fontSize: 11, color: 'var(--cth-ink-300)' }}>… ほか{stuck.length - 6}件</div>
                  )}
                </div>
              )}
            </div>
